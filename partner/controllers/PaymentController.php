    ...
    public function actionCheck()
    {
	    ...
	    if ($order->status == Order::OS_PAYED) {
		    return $this->renderPartial('check', [
			    ...
			    'order' => $order,
			    'post' => $req->post(),
			    'message' => \Yii::t('payment', 'This order already payed', $order->lang),
		    ]);
		    ...
		    return $this->renderPartial('check', [
			    ...
			    'order' => $order,
			    'post' => $req->post(),
			    'message' => \Yii::t('payment', 'Wrong order status', $order->lang),
		    ]);
	    }
	    ...
	    if ($order->pay_sum != $pay->orderSumAmount) {
		    return $this->renderPartial('check', [
			    ...
			    'order' => $order,
			    'post' => $req->post(),
			    'message' => \Yii::t('payment', 'Wrong order sum', $order->lang),
		    ]);
	    }
	    ...
	    if ($pay->validate() && $pay->save(false)) {
		    ...
		    return $this->renderPartial('check', [
			    ...
			    'order' => $order,
			    'post' => $req->post(),
			    'message' => \Yii::t('payment', 'Internal server error', $order->lang),
		    ]);
	    }
    }
