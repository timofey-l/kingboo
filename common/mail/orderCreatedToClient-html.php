...
<div>
    ...
    <p><?= \Yii::t('mails_order', 'You made a order on the site <a href="https://king-boo.com">king-boo.com</a>.', [], $local) ?></p>
    ...
    <?php if (!$order->payment_via_bank_transfer && !$order->checkin_fullpay): ?>
        ...
        <p><?= \Yii::t('mails_order', 'To make a payment, please click the link below <br/><a href="{url}">{url}</a>', ['url' => 'https://king-boo.com/payment/' . $order->payment_url], $local) ?></p>
    <?php endif; ?>

    <?php // оплата при заселении ?>
    <?php if ($order->checkin_fullpay): ?>
        <p><?= \Yii::t('mails_order', 'Booking is successfully complete. You need to make the full payment at check in  - {sum}.', ['sum' => $currency->getFormatted($order->sum)]) ?></p>
    <?php endif; ?>
